...
int main(int argc, char* argv[])
{
    ...
    // cscene.push_back(Circle(40, 200,  40, 4, 100));
    // cscene.push_back(Circle(400, 200, 40, 4, -100));
    //cscene.push_back(Circle(40, 150, 40,4, 100));

    float r;
    for (int i = 0; i < 100; i++)
        cscene.push_back(Circle((i * 25) % (width - 25) + 25, 25 + ((i * 25) / width), 20, 4, rand() % 800 - 400, rand() % 1600 - 800));



    // for (int i = 0; i < 100; i++)
    //     scene.push_back(Box(rand() % 4 + 1, (i % 50) * 25, int(i / 50) * 25 , 20, 20, true, rand() % 2 + 1, rand() % 2 + 1));
    ...
    SDL_CreateWindowAndRenderer(width, height, SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN , &win, &ren);
